...
class Qwen2:
    ...
    def __init__(self, model_path, device: DeviceType = DeviceType.CPU):
        ...
        self.model = LIB_LLAISYS.llaisysQwen2ModelCreate(byref(self.meta), device, device_ids, len(device_ids))

        self.weights = LIB_LLAISYS.llaisysQwen2ModelWeights(self.model)

        LIB_LLAISYS.tensorLoad(getattr(self.weights.contents, "in_embed"), state_dict["model.embed_tokens.weight"].data_ptr())
        LIB_LLAISYS.tensorLoad(getattr(self.weights.contents, "out_embed"), state_dict["lm_head.weight"].data_ptr())
        LIB_LLAISYS.tensorLoad(getattr(self.weights.contents, "out_norm_w"), state_dict["model.norm.weight"].data_ptr())
    ...
    def generate(
        ...
        past_len = c_size_t(0)
        next_token = LIB_LLAISYS.llaisysQwen2ModelInfer(self.model, token_ids, ntoken, past_len)
        ...
        for _ in range(max_new_tokens - 1):
            if next_token == self.meta.end_token:
                break
            ntoken = 1
            token_ids = (c_int64 * ntoken)(next_token)
